...
\large Wide-Scale Sampling with Hard-Argmin Selection and CBF Projection
...
Multiple-shooting with CasADi Opti + IPOPT, using identical $Q, R$
weights and soft obstacle penalty ($\rho = 2000$, $\delta = 0.3$\,m)
as TSH-NMPC (Eq.~6, main text). Horizons: $H = 10$ ($0.2$\,s) and
$H = 20$ ($0.4$\,s). Unlike TSH-NMPC, no separate CBF projection;
the soft obstacle penalty is part of the NLP cost. Latency includes
full Opti stack overhead and IPOPT solve on the same single-threaded
x86 CPU.
