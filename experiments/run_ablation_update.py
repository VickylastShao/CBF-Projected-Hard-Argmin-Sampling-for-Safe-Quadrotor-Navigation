# -*- coding: utf-8 -*-
"""
Update ablation tables (E.1, E.2, E.3) with retrained TRM model.
"""
...
def main():
    ...
    # ========== E.1: Candidate Generation Mode Ablation ==========
    print("\n" + "=" * 80)
    print("E.1: Candidate Generation Mode Ablation (K=50)")
    print("=" * 80)

    e1 = {}

    # CL-TRM+PD(alpha=0.5)+Rollout — uses candidate_mode='trm_pd' for proper TRM+PD blending
    set_seed(SEED)
    ...
    # PD+Rollout (alpha=1.0) — pure PD baseline
    set_seed(SEED)
    predictor = PTRMNMPCPredictor(trm_model, env, K=50, D=16, sigma=0.25,
    ...
    # CL-TRM+Rollout (alpha=0.0) K=50 — pure TRM generation
    set_seed(SEED)
    predictor = PTRMNMPCPredictor(trm_model, env, K=50, D=16, sigma=0.25,
    ...
    # CL-TRM+Rollout (alpha=0.0) K=1 — pure TRM deterministic
    set_seed(SEED)
    ...
    # ========== E.2: Evaluation Strategy Ablation ==========
    print("\n" + "=" * 80)
    print("E.2: Evaluation Strategy Ablation (K=50)")
    print("=" * 80)

    e2 = {}

    # Pure Rollout (proposed) - alpha=0.3, rollout
    set_seed(SEED)
    predictor = PTRMNMPCPredictor(trm_model, env, K=50, D=16, sigma=0.25,
    ...
    r = run_mc_trials(env, predictor, X_SP, enable_cbf=True)
    e2['pure_rollout'] = r
    print(f"  Pure Rollout:     Succ={r['success_rate']:.0f}%, TErr={r['terminal_error_mean']:.4f}±{r['terminal_error_std']:.4f}m, IAE={r['iae_mean']:.1f}")

    # Q-head + Rollout (legacy) - alpha=0.3, ranking_mode='q_head' (default)
    set_seed(SEED)
    predictor = PTRMNMPCPredictor(trm_model, env, K=50, D=16, sigma=0.25,
                                   alpha_blend=0.3, candidate_mode='pd',
                                   pd_sigma=2.0, use_rollout_cost=True,
    ...
    r = run_mc_trials(env, predictor, X_SP, enable_cbf=True)
    e2['q_head_rollout'] = r
    print(f"  Q-head+Rollout:   Succ={r['success_rate']:.0f}%, TErr={r['terminal_error_mean']:.4f}±{r['terminal_error_std']:.4f}m, IAE={r['iae_mean']:.1f}")

    # Random selection - uses ranking_mode='random'
    set_seed(SEED)
    predictor = PTRMNMPCPredictor(trm_model, env, K=50, D=16, sigma=0.25,
                                   alpha_blend=0.3, candidate_mode='pd',
                                   pd_sigma=2.0, use_rollout_cost=True,
    ...
    r = run_mc_trials(env, predictor, X_SP, enable_cbf=True)
    e2['random'] = r
    print(f"  Random selection:  Succ={r['success_rate']:.0f}%, TErr={r['terminal_error_mean']:.4f}±{r['terminal_error_std']:.4f}m, IAE={r['iae_mean']:.1f}")
    ...
    r = run_mc_trials(env, predictor, X_SP, enable_cbf=True)
    e2['pd_k1'] = r
    print(f"  PD K=1:           Succ={r['success_rate']:.0f}%, TErr={r['terminal_error_mean']:.4f}±{r['terminal_error_std']:.4f}m, IAE={r['iae_mean']:.1f}")

    all_results['e2'] = e2

    # ========== E.3: High-Noise Robustness Ablation ==========
    print("\n" + "=" * 80)
    print("E.3: High-Noise Robustness Ablation (with mismatch)")
    print("=" * 80)

    e3 = {}

    # PD K=1 at various noise levels
    for noise in [0.0, 0.01, 0.05, 0.10]:
        ...
        e3[f'pd_k1_n{noise}'] = r
        print(f"  PD K=1 noise={noise:.2f}:    Succ={r['success_rate']:.0f}%, TErr={r['terminal_error_mean']:.4f}m, IAE={r['iae_mean']:.1f}")

    # CL-TRM+PD K=50 at various noise levels
    for noise in [0.0, 0.01, 0.05, 0.10]:
        set_seed(SEED)
        predictor = PTRMNMPCPredictor(trm_model, env, K=50, D=16, sigma=0.25,
                                       alpha_blend=0.3, candidate_mode='pd',
        ...
        r = run_mc_trials(env, predictor, X_SP, enable_cbf=True,
                          use_mismatch=True, process_noise=noise)
        e3[f'trm_pd_k50_n{noise}'] = r
        print(f"  CL-TRM+PD K=50 noise={noise:.2f}: Succ={r['success_rate']:.0f}%, TErr={r['terminal_error_mean']:.4f}m, IAE={r['iae_mean']:.1f}")

    all_results['e3'] = e3

    # Save results
    out_path = os.path.join(save_dir, 'retrain_ablation_update.json')
